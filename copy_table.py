...
types = {
    ...
    'taxo': {
         ...
         2: { 'en':'[[Image:',                        'de':'[[Bild:',                                     'nl':'[[Afbeelding:',                                  },
         3: { 'en':'[[Scientific classification]]',   'de':'[[Systematik (Biologie)|Systematik]]',        'nl':'[[Taxonomie|Wetenschappelijke  classificatie]]', },
         4: { 'en':'[[Domain (biology)|Domain]]',     'de':u'\'\'[[Dom\xe4ne (Biologie)|Dom\xe4ne]]\'\'', 'nl':'[[Domain (biologie)|Domain]]'                    },
         5: { 'en':'[[Kingdom (biology)|Kingdom]]',   'de':'\'\'[[Reich (Biologie)|Reich]]\'\'',          'nl':'[[Rijk (biologie)|Rijk]]',                       },
         6: { 'en':'[[Phylum (biology)|Phylum]]',     'de':'\'\'[[Stamm (Biologie)|Stamm]]\'\'',          'nl':'[[Stam (biologie)|Stam]]',                       },
         7: { 'en':'[[Subphylum]]',                   'de':'\'\'[[Unterstamm]]\'\'',                      'nl':'[[Substam (biologie)|Substam]]',                 },
         8: { 'en':'[[Class (biology)|Class]]',       'de':'\'\'[[Klasse (Biologie)|Klasse]]\'\'',        'nl':'[[Klasse (biologie)|Klasse]]',                   },
         9: { 'en':'[[Subclass (biology)|Subclass]]', 'de':'\'\'[[Klasse (Biologie)|Unterklasse]]\'\'',   'nl':'[[Onderklasse]]',                                },
        10: { 'en':'[[Order (biology)|Order]]',       'de':'\'\'[[Ordnung (Biologie)|Ordnung]]\'\'',      'nl':'[[Orde (biologie)|Orde]]',                       },
        11: { 'en':'[[Family (biology)|Family]]',     'de':'\'\'[[Familie (Biologie)|Familie]]\'\'',      'nl':'[[Familie (biologie)|Familie]]',                 },
        12: { 'en':'[[Genus|Genera]]',                'de':'\'\'[[Gattung (Biologie)|Gattung]]\'\'',      'nl':'[[Geslacht (biologie)|Geslacht]]',               },
        13: { 'en':'[[Species]]',                     'de':'\'\'[[Art (Biologie)|Art]]\'\'',              'nl':'[[Soort]]',                                      },
        }
    }
...
class NoInterwiki(Exception):
    """Interwiki link to specified language not found."""
...
def treat(to_pl):
    ...
    if from_pl == '':
        ...
        raise NoInterwiki('Interwiki link not found.')
    try:
        interwikis = to_pl.interwiki()
    except wikipedia.IsRedirectPage:
        pass
...
def translate(text):
    if type == '':
        ...
        if translation.get(1).has_key(from_lang) and translation.get(1).has_key(wikipedia.mylang):
            if debug: print 'Both "from" and "to" language were found.'
        else:
            print 'Can\'t translate. Please make sure that there are entries for ' + translation.get(1).has_key(from_lang) + ' and ' + this_type.get(1).has_key(wikipedia.mylang) + 'in copy_table.py.'
            pass
        for i in range(len(translation)):
            if debug: print i + ': ' + translation.get(i).get(from_lang) + '=>' + translation.get(i).get(wikipedia.mylang)
